    ...
    <div className="flex gap-4 h-[calc(100vh-80px)]">
      <div className="w-48 shrink-0 bg-white rounded-2xl border border-border p-3 space-y-1 overflow-y-auto">
        <div className="px-3 py-2 mb-2">
          <p className="font-display text-sm font-bold text-text-primary">Categories</p>
        </div>
        <button
          onClick={() => handleCategoryClick(null)}
          className={`w-full text-left px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${
            !selectedCat ? 'bg-brand-main text-white' : 'text-text-secondary hover:bg-surface-2'
          }`}
        >
          All Products
        </button>
        {categories.map((cat) => (
          <button
            ...
            className={`w-full text-left px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${
              selectedCat === cat.id ? 'bg-brand-main text-white' : 'text-text-secondary hover:bg-surface-2'
            }`}
          >
            {cat.icon} {cat.name}
          </button>
        ...
        <div className="flex-1 flex flex-col relative pb-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 overflow-y-auto">
              ...
              return (
                <motion.div key={product.id} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                  <Card className="p-4 cursor-pointer relative" hover onClick={() => addToCart(product)}>
                    {inCart && (
                      <span className="absolute -top-2 -right-2 w-6 h-6 bg-brand-main text-white text-xs font-bold rounded-full flex items-center justify-center">
                        {inCart.quantity}
                      </span>
                    )}
                    {productImage ? (
                      ...
                      <div className="w-full h-24 rounded-lg mb-2 bg-surface-2" />
                    )}
            ...
            {products.length === 0 && !loading && (
              <div className="col-span-full text-center py-10 text-text-muted">No products found.</div>
            )}
              ...
              <div key={item.productId} className="flex items-center gap-3 py-2 border-b border-surface-2 last:border-0">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="w-6 h-6 rounded-lg bg-surface-2 flex items-center justify-center hover:bg-danger-pale"
                  >
                  ...
                  <button
                    onClick={() => updateQty(item.productId, 1)}
                    className="w-6 h-6 rounded-lg bg-surface-2 flex items-center justify-center hover:bg-brand-pale"
                  >
